...
# Teardown context: Close SQLAlchemy session after each request
...
# Route for custom template rendering
@app.route('/4-hbnb')
def hbnb_filters(the_id=None):
    ...
    # Retrieve all Amenity objects from the database
    amens = storage.all('Amenity').values()
    
    # Retrieve all Place objects from the database
    ...
    return render_template('4-hbnb.html',
                           cache_id=uuid.uuid4(),  # Generate unique cache ID for cache busting
                           states=states,
                           amens=amens,
                           places=places,
                           users=users)
